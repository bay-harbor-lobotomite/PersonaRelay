...
const ChatInput: React.FC<ChatInputProps> = ({ onSendMessage, isSending }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-center p-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
      ...
      <button type="submit" className="ml-4 p-3 bg-blue-500 text-white rounded-full hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 disabled:opacity-50" disabled={!input.trim() || isSending}><SendIcon className="w-5 h-5" /></button>
    </form>
  );
}
