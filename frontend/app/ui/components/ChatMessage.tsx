...
const ChatMessage = ({ message }: { message: Message }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`px-4 py-3 rounded-2xl max-w-[70%] sm:max-w-[40%] ${isUser ? 'bg-blue-500 text-white rounded-br-none' : 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-bl-none'}`} style={isUser ? userMessage : botMessage}><p className="text-sm">{message.text}</p></div>
      {isUser && <div className="flex-shrink-0 w-10 h-10 rounded-fullflex items-center justify-center"><UserIcon className="w-6 h-6 text-gray-500 dark:text-gray-400"/></div>}
    </motion.div>
  );
}
